...
class MistralForCausalLM(MistralPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        weight_masks = kwargs["weight_masks"].detach()
        ...
        if labels is not None:
            ...
            shift_weight_masks = weight_masks[..., 1:].contiguous()
            # Flatten the tokens
            shift_logits = shift_logits.view(-1, self.config.vocab_size)
            shift_labels = shift_labels.view(-1)
            # Ensure tensors are on the same device
            shift_labels = shift_labels.to(shift_logits.device)
            shift_weight_masks = shift_weight_masks.view(-1)
            shift_weight_masks = shift_weight_masks.to(shift_logits.device)
            loss_fct = CrossEntropyLoss(reduction='none')
            loss = loss_fct(shift_logits, shift_labels)
            loss = loss * shift_weight_masks
            loss = loss.mean()
